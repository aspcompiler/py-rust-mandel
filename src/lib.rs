use ndarray::{ArrayViewMut, Dim, IxDynImpl};
...
fn mandelbrot(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(compute_mandelbrot_rs_simd, m)?)?;
    Ok(())
}
...
fn compute_mandelbrot_rs_simd(
    ...
    iters: u32,
    parallel: bool,
) -> &PyArray<u32, Dim<[usize; 2]>> {
    let out = simd::generate(min_x, max_x, min_y, max_y, width, height, iters, parallel);
...
fn compute_mandelbrot(
    ...
    height: u32,
    iters: u8,
    t: &mut ArrayViewMut<'_, u8, Dim<IxDynImpl>>,
) {
